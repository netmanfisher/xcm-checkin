...
-- 1. 插入管理员（密码: admin123）
INSERT INTO xcm_admins (username, password_hash)
VALUES ('admin', '7b6b5e9e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e8e')
ON CONFLICT (username) DO NOTHING;

-- 2. 插入测试孩子（简化版，不包含可能不存在的字段）
INSERT INTO xcm_children (name, avatar_url, theme, status, stars, created_at)
VALUES
  ('小虾米', '👧', 'princess', 'active', 50, NOW()),
  ('小明', '👦', 'prince', 'active', 30, NOW())
...
-- 6. 为小虾米初始化连续打卡记录（如果表存在）
DO $$
BEGIN
  -- 检查表是否存在
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'xcm_streaks') THEN
...
BEGIN
  ...
  IF v_child_id IS NOT NULL THEN
    ...
    -- 插入昨天的已审核打卡记录
    IF v_plan_piano IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_piano, 'approved', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day');
    END IF;

    IF v_plan_homework IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_homework, 'approved', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day');
    END IF;

    IF v_plan_reading IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_reading, 'approved', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day');
    END IF;

    -- 插入前天的已审核打卡记录
    IF v_plan_piano IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_piano, 'approved', NOW() - INTERVAL '2 days', NOW() - INTERVAL '2 days');
    END IF;

    IF v_plan_homework IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_homework, 'approved', NOW() - INTERVAL '2 days', NOW() - INTERVAL '2 days');
    END IF;

    -- 插入今天的已审核打卡记录
    IF v_plan_reading IS NOT NULL THEN
      INSERT INTO xcm_check_ins (child_id, plan_id, status, check_in_time, reviewed_at)
      VALUES (v_child_id, v_plan_reading, 'approved', NOW(), NOW());
    END IF;
  END IF;
END $$;
...
BEGIN
  ...
  IF v_child_id IS NOT NULL AND v_achievement_first IS NOT NULL THEN
